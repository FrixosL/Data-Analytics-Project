...
I. Data Wrangling: For further analysis, we replaced missing values, recategorized property types, and defined region based on average price of neighbourhood.  
II. Data Visualization: We explored relationship between key variables through histogram, correlation matrix and an interactive map.  
III. Model Building: To find the best fit model for explained variable “log_price_4_nights”, we conducted regression analysis using different explanatory variables. Our final model contains 13 explanatory variables, with adjusted R2 of 55.54%.  
IV. Model Significance and Diagnostics: We further checked significance, residuals and collinearity of the final model.  
V. Prediction: Finally, we leveraged the best model to predict the total cost of 4-night stay at an Airbnb, which has a private room, 10 reviews, and an average rating of 90.  
...
To solve this problem, we imported the relevant Airbnb data in Barcelona and conducted Exploratory Data Analysis (EDA) first. 
...
In order to get clean data for further analysis, we processed it in following steps:  
1) Filter data used for travel purposes only, using ‘minimum_nights <= 4’
2) Select explanatory variables for price.  
3) Replace missing value of ‘cleaning_fee’ with 0.  
4) Simplify ‘property_type’ variable, as majority of the observations in the data are one of the top four or five property types.  
5) Group neighbourhood to 3 regions based on geography and average prices in each neighbourhood.  
6) Calculate explained variable: price for 4 nights for two people and its logarithm.
